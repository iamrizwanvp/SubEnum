...
# Final cleanup with urldedupe (smart deduplication)
echo "[INFO] Removing duplicates and normalizing URLs using urldedupe..."
cat "$output_file" | urldedupe -s > deduped_urls.txt

# Replace old output with the cleaned version
mv deduped_urls.txt "$output_file"
